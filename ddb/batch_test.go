...
func TestBatchGetItemProjectionRejected(t *testing.T) {
	c := newClient(t)
	ctx := context.Background()
	mustCreateTable(t, c, "T")
	...
	cases := []struct {
		...
		{"ProjectionExpression", KeysAndAttributes{Keys: key, ProjectionExpression: "pk"}},
		{"ExpressionAttributeNames", KeysAndAttributes{Keys: key, ExpressionAttributeNames: map[string]string{"#p": "pk"}}},
		{"AttributesToGet", KeysAndAttributes{Keys: key, AttributesToGet: []string{"pk"}}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			_, err := c.BatchGetItem(ctx, BatchGetItemInput{RequestItems: map[string]KeysAndAttributes{"T": tc.ka}})
			if !errors.Is(err, ErrValidation) {
				t.Errorf("err = %v, want ErrValidation", err)
			}
		})
