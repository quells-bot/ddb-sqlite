...
func (c *Client) BatchGetItem(ctx context.Context, in BatchGetItemInput) (BatchGetItemOutput, error) {
	...
	for table, ka := range in.RequestItems {
		...
		// Projection is a v1 non-goal: rejected, not silently ignored (spec
		// §1.1.3 — deliberate divergence from dynamodb-local).
		if ka.ProjectionExpression != "" || len(ka.ExpressionAttributeNames) > 0 || len(ka.AttributesToGet) > 0 {
			return BatchGetItemOutput{}, fmt.Errorf("%w: ProjectionExpression, ExpressionAttributeNames, and AttributesToGet are not supported", ErrValidation)
		}
		...
		prepared[table] = tablePlan{def: def, vals: vals}
	}
